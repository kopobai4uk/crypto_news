...
class CryptoNewsSpider(scrapy.Spider):
    ...
    def parse_list_of_news(self, response):
        ...
        if response.xpath('//script/text()').re(".*_load_more.*"):
            ...
            js_dict = chompjs.parse_js_object(script_function)
            formatted_data_form_script = lxml.html.fromstring(js_dict['pages'][0])
            links_of_news_links.extend(formatted_data_form_script.xpath('//div/a/@href'))
            list_of_date_posted_news.extend(formatted_data_form_script.xpath('//time/@datetime'))
    ...
    def parse_news(self, response):
        ...
        news_item['author'] = []
        news_item['author'].extend(response.xpath('//div[contains(@class,"cn-props-panel")]'
