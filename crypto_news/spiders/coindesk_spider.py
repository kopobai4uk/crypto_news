...
class CoinDeskSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        list_of_groups_links = response.xpath('//div'
    ...
    def parse_list_news_links(self, response):
        yield response.follow(self.pagination_url + urlparse(response.url).path
                              + '/1?mode=list', self.parse_list_news_links_pagination)

    def parse_list_news_links_pagination(self, response):

        page_response_json = json.loads(response.body.decode('UTF-8'))
        for post in page_response_json['posts']:
            ...
            item['date'] = post['date']
            slug = post['slug']
            yield scrapy.Request(self.start_urls[0]+slug, self.parse_news,
                                 meta={'item': item})
        ...
        yield scrapy.Request(self.start_urls[0][:-1] + url.path[:-1]
    ...
    def parse_news(self, response):
        ...
        item['text'] = response.xpath('//section[contains(@class, "article-body")]'
                                      '/div/p/text()').getall()
        yield item